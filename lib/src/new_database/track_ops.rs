...
pub fn count_all_track_metadata(conn: &Connection) -> Result<Integer> {
    let count = conn.query_row("SELECT COUNT(file) FROM tracks_metadata;", [], |v| v.get(0))?;
...
mod tests {
    ...
    use crate::{
        new_database::{
            ...
            track_ops::{
                all_distinct_directories,  get_all_tracks, get_last_position,
                get_track_from_path, get_tracks_from_album, get_tracks_from_artist,
                get_tracks_from_directory, get_tracks_from_genre, 
                 set_last_position, track_exists, AlbumRead, ArtistRead,
                RowOrdering, TrackRead,
            },
        },
        track::TrackMetadata,
    };
