...
CREATE TABLE IF NOT EXISTS tracks_metadata(
    ...
    file INTEGER PRIMARY KEY NOT NULL UNIQUE REFERENCES tracks(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS tracks_artists(
    ...
    PRIMARY KEY (file, artist)
);

-- the table for all albums
-- Note: not unique, there *could* be ones with the same name
-- Note: "artist_display" or "albums_artist" should be used to match exact album
