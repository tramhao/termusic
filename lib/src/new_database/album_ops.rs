...
use crate::new_database::{
    artist_ops::{common_row_to_artistread, ArtistRead},
    Integer,
};
...
/// Count all rows currently in the `albums_artist` database
#[cfg(test)]
pub(super) fn count_all_albums_artist_mapping(conn: &Connection) -> Result<Integer> {
    let count = conn.query_row("SELECT COUNT(album) FROM albums_artist;", [], |v| v.get(0))?;
...
pub fn get_all_artists_for_album(conn: &Connection, album_id: Integer) -> Result<Vec<ArtistRead>> {
    let mut stmt = conn.prepare(indoc! {"
        ...
        INNER JOIN albums_artist ON albums_artist.album=(:album_id)
        WHERE artists.id=albums_artist.artist;
    "})?;
...
mod tests {
    use crate::new_database::{
        ...
        album_ops::{
            album_exists, get_all_albums, get_all_albums_like, get_all_artists_for_album,
            AlbumRead, RowOrdering,
        },
