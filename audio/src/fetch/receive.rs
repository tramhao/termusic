...
use crate::{
    fetch::PREFETCH_THRESHOLD_FACTOR,
    range_set::{Range, RangeSet},
};

use super::{
    client::Client, AudioFileError, AudioFileResult, AudioFileShared, StreamLoaderCommand,
    StreamingRequest, MAXIMUM_ASSUMED_PING_TIME, MINIMUM_DOWNLOAD_SIZE, MINIMUM_THROUGHPUT,
};
...
impl AudioFileFetch {
    ...
    fn download_range(&mut self, offset: usize, mut length: usize) -> AudioFileResult {
        ...
        for range in ranges_to_request.iter() {
            let streamer =
                Client::new().stream_from_url(&self.shared.url, range.start, range.length)?;
