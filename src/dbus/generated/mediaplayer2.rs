    ...
    for dbus::ConnPath<'a, C>
{
    ...
    fn raise(&self) -> Result<(), Self::Err> {
        let mut m =
            self.method_call_with_args(&"org.mpris.MediaPlayer2".into(), &"Raise".into(), |_| {})?;
        m.as_result()?;
        Ok(())
    }
    ...
    fn quit(&self) -> Result<(), Self::Err> {
        let mut m =
            self.method_call_with_args(&"org.mpris.MediaPlayer2".into(), &"Quit".into(), |_| {})?;
        m.as_result()?;
        Ok(())
    }
...
{
    let interface = factory.interface("org.mpris.MediaPlayer2", data);
    ...
    let method = factory.method("Raise", Default::default(), handle);
    ...
    let method = factory.method("Quit", Default::default(), handle2);
    let interface = interface.add_m(method);
