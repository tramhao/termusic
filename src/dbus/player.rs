use crate::dbus::generated::mediaplayer2::org_mpris_media_player2_server;
use crate::dbus::generated::mediaplayer2_player::{
    org_mpris_media_player2_player_server, OrgFreedesktopDBusPropertiesPropertiesChanged,
    OrgMprisMediaPlayer2Player,
};
use crate::dbus::metadata::Metadata;
use crate::dbus::status::{Loop, Playback};
use crate::dbus::OrgMprisMediaPlayer2;
...
type MyType<T> = RefCell<Vec<Rc<RefCell<T>>>>;
type MyReturn = HashMap<String, Variant<Box<dyn RefArg + 'static>>>;
pub struct Mpris {
    ...
    playback_status: Cell<Playback>, // R
    loop_status: Cell<Loop>,         // R/W
    rate: Cell<f64>,                 // R/W
    ...
    // Callbacks
    raise_cb: MyType<dyn FnMut()>,
    quit_cb: MyType<dyn FnMut()>,
    next_cb: MyType<dyn FnMut()>,
    previous_cb: MyType<dyn FnMut()>,
    pause_cb: MyType<dyn FnMut()>,
    play_pause_cb: MyType<dyn FnMut()>,
    stop_cb: MyType<dyn FnMut()>,
    play_cb: MyType<dyn FnMut()>,
    seek_cb: MyType<dyn FnMut(i64)>,
    open_uri_cb: MyType<dyn FnMut(&str)>,
    fullscreen_cb: MyType<dyn FnMut(bool)>,
    loop_status_cb: MyType<dyn FnMut(Loop)>,
    rate_cb: MyType<dyn FnMut(f64)>,
    shuffle_cb: MyType<dyn FnMut(bool)>,
    volume_cb: MyType<dyn FnMut(f64)>,
}

#[allow(unused)]
impl Mpris {
    pub fn new(mpris_name: &str, identify: &str, desktop_entry: &str) -> Arc<Self> {
        let connection = Arc::new(Connection::get_private(BusType::Session).unwrap());
        let factory = Arc::new(Factory::new_fn());

        let mpris_player = Arc::new(Self {
            connection,
            factory,
            ...
            playback_status: Cell::new(Playback::Paused),
            loop_status: Cell::new(Loop::None),
            rate: Cell::new(0_f64),
        ...
        let root_iface: Interface<MTFn<TData>, TData> =
            org_mpris_media_player2_server(&mpris_player.factory, (), |m| {
                let a: &Arc<Self> = m.path.get_data();
                let b: &Self = a;
                b
            });
    ...
    pub fn set_playback_status(&self, value: Playback) {
    ...
    pub fn set_loop_status(&self, value: Loop) {
    ...
    pub fn set_minimum_rate(&self, value: f64) {
        if (self.minimum_rate.get() - value).abs() > 0.001 {
            //} != value {
    ...
    pub fn set_maximum_rate(&self, value: f64) {
        if (self.maximum_rate.get() - value).abs() > 0.001 {
            //}!= value {
    ...
    pub fn connect_loop_status<F: FnMut(Loop) + 'static>(&self, callback: F) {
        let cell = Rc::new(RefCell::new(callback));
...
impl OrgMprisMediaPlayer2 for Mpris {
...
impl OrgMprisMediaPlayer2Player for Mpris {
    type Err = tree::MethodErr;
    ...
    fn get_metadata(&self) -> Result<MyReturn, Self::Err> {
