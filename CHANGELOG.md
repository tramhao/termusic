...
### [v0.13.0]
- Yanked due to dependency lofy was yanked in crates.io
...
- Fix(tui): in Music Library, when pressing `RIGHT` on a non-loaded directory, will now load the directory (without having to step-into it).
  - Note that non-loaded and empty directories currently do not draw differently than files.
...
- Fix(server): round MPRIS volume change to nearest integer (so that `0.4999` does not become `4` but `5`).
